package data;
// Generated 20/09/2017 9:54:50 PM by Hibernate Tools 5.2.3.Final
...
//			return (SessionFactory) new InitialContext(prop).lookup("SessionFactory");
	...
	public Emp findById(java.lang.Integer id) {
		...
		try {
			Emp instance = (Emp) sessionFactory.getCurrentSession().get("data.Emp", id);
			if (instance == null) {
	...
	public List findByExample(Emp instance) {
		...
		try {
			List results = sessionFactory.openSession().createCriteria("data.Emp").add(Example.create(instance))
//			List results = sessionFactory.getCurrentSession().createCriteria("data.Emp").add(Example.create(instance))
					.list();
